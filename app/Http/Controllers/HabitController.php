...
class HabitController extends Controller
{
    public function dashboard()
    {
        ...
        $habits = auth()->user()->habits()->with(['category', 'completions' => function ($query) use ($userTimezone) {
            $query->where('completed_date', '>=', now()->timezone($userTimezone)->subDays(6)->format('Y-m-d'));
        }])->get();

        $days = [];
        for ($i = 6; $i >= 0; $i--) {
            $date = now()->timezone($userTimezone)->subDays($i);
            $days[] = [
        ...
        $todayDate = now()->timezone($userTimezone)->format('Y-m-d');

        return view('dashboard', compact('habits', 'days', 'todayDate'));
    }
    ...
    public function toggle(Request $request, Habit $habit)
    {
        ...
        $userTimezone = auth()->user()->timezone ?? 'UTC';
        $date = $request->input('date', now()->timezone($userTimezone)->format('Y-m-d'));
