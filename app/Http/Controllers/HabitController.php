...
class HabitController extends Controller
{
    public function dashboard()
    {
        ...
        $habits = auth()->user()->habits()->with(['category', 'completions' => function ($query) use ($userTimezone) {
            $query->where('completed_date', '>=', now($userTimezone)->subDays(6)->format('Y-m-d'));
        }])->get();

        $days = [];
        for ($i = 6; $i >= 0; $i--) {
            $date = now($userTimezone)->subDays($i);
            $days[] = [
    ...
    public function toggle(Request $request, Habit $habit)
    {
        ...
        $userTimezone = auth()->user()->timezone ?? 'UTC';
        $date = $request->input('date', now($userTimezone)->format('Y-m-d'));
