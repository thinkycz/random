...
class HabitController extends Controller
{
    public function dashboard()
    {
        ...
        $habits = auth()->user()->habits()->with(['category', 'completions' => function ($query) use ($tz) {
            $query->where('completed_date', '>=', now($tz)->subDays(6)->format('Y-m-d'));
        }])->get();
    ...
    public function toggle(Request $request, Habit $habit)
    {
        ...
        $tz = auth()->user()->timezone ?? 'UTC';
        $date = $request->input('date', now($tz)->format('Y-m-d'));
