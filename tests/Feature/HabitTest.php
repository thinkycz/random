...
class HabitTest extends TestCase
{
    ...
    public function test_timezone_day_boundary()
    {
        // America/New_York is UTC-4 or UTC-5 depending on daylight savings.
        $user = \App\Models\User::factory()->create(['timezone' => 'America/New_York']);
        $habit = \App\Models\Habit::factory()->create(['user_id' => $user->id]);

        // Mock time to 2 AM UTC on March 9th.
        // In New York, it should still be 10 PM (or 9 PM) on March 8th.
        \Carbon\Carbon::setTestNow('2026-03-09 02:00:00');

        $response = $this->actingAs($user)->post("/habits/{$habit->id}/toggle");

        // It should record the completion for March 8th, not March 9th, because in NY it's still March 8.
        $this->assertDatabaseHas('habit_completions', [
            'habit_id' => $habit->id,
            'completed_date' => '2026-03-08 00:00:00',
        ]);

        // Assert streaks are calculated correctly
        $streaks = $habit->fresh()->getStreaks();
