...
class HabitTest extends TestCase
{
    ...
    public function test_habit_calculations_respect_user_timezone(): void
    {
        // Find a timezone that is definitely on a different day than UTC right now
        // Or we can mock the time. Let's mock the time to be exactly at UTC midnight.
        // At UTC 00:00:00, it is 16:00:00 the *previous* day in America/Los_Angeles.
        $knownDate = \Carbon\Carbon::create(2024, 1, 2, 0, 0, 0, 'UTC');
        \Carbon\Carbon::setTestNow($knownDate);

        // Create user with a timezone behind UTC
        $user = \App\Models\User::factory()->create(['timezone' => 'America/Los_Angeles']);
        $habit = \App\Models\Habit::factory()->create(['user_id' => $user->id]);

        // In UTC it is 2024-01-02.
        // In LA it is 2024-01-01.
        $laToday = '2024-01-01';

        // Ensure the toggle endpoint uses LA's timezone to determine "today" when no date is passed
        $response = $this->actingAs($user)->post("/habits/{$habit->id}/toggle");

        $this->assertDatabaseHas('habit_completions', [
            'habit_id' => $habit->id,
            'completed_date' => $laToday . ' 00:00:00',
        ]);

        // Test streak calculations
        // LA today is 2024-01-01, so there is 1 completion for "today"
        $streaks = $habit->fresh()->getStreaks();
