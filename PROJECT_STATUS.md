...
**Summary:** Implemented user timezone handling. Users can now select their local timezone in their profile. Habit completion toggles and streak calculations now respect this local timezone, resolving the issue where server time could misattribute habits to the wrong day.
...
- Implemented user timezone preference and timezone-aware habit tracking.
...
- **Refactoring:** Extract completion logic into a dedicated Service or Action class as it is growing more complex.
...
- Added specific tests for timezone-aware streak calculations (`test_streak_calculation_respects_timezone`).
