...
**Summary:** Added timezone support for users. Users can now select their local timezone in their profile settings. The application uses this timezone to correctly determine "today", resolving issues with habit completions and streaks for users in different timezones.
...
- Added user timezone support to handle "today" calculation based on user's timezone rather than server timezone.
