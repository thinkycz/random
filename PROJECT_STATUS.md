...
**Summary:** Implemented User Timezone support. Added a `timezone` field to the users table and updated the profile page to allow users to select their timezone. Updated the `HabitController` and `Habit` model's streak logic to calculate "today" and "yesterday" boundaries based on the user's selected timezone rather than UTC, resolving the bug where habits could be logged on incorrect dates.
...
- Implemented User Timezone support, allowing users to select their timezone and correctly mapping completion dates and streaks to their local time.
