...
**Summary:** Added Client-Side Timezone Handling. Users can now select their timezone in their profile, and "today" calculations on the dashboard, as well as streak calculations, now use their configured timezone instead of the server's UTC time.
...
- Added a `timezone` column to the `users` table and updated profile settings to let users define their local timezone. Updated habit completion and streak logic to rely on the user's timezone.
