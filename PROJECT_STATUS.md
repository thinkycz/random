...
**Summary:** Added timezone support for users. Users can now select their timezone in their profile settings. Dashboard dates, completions, and streak calculations now properly respect the user's local timezone rather than relying strictly on the server's UTC time.
...
- Fixed the timezone bug where completions and dates didn't align with users outside of UTC.
- Added extensive feature tests for timezone behaviors.
...
- The "toggle" logic in `HabitController` works but handles several responsibilities. Consider extracting this logic as the app grows.
...
- Extensive tests added for Timezone operations (`TimezoneTest.php`) verifying streak calculation and dashboard respect timezones.
