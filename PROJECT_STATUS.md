...
**Summary:** Added client-side timezone handling. Users can now select their local timezone from the profile page, ensuring "today" and streaks align correctly with their local time, regardless of the server's timezone.
...
- Added `timezone` column to `users` table and integrated into profile settings.
...
- Basic feature tests cover:
  ...
  - Timezone effects on dashboard display and toggles.
