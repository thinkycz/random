...
**Summary:** Fixed the timezone issue. Added a `timezone` field to the `users` table and the profile update form. The application now uses the user's local timezone when calculating 'today' for the dashboard and determining streak counts.
...
- Fixed timezone bug by storing user timezone and applying it to date-based queries and streak calculations.
