...
**Summary:** Fixed the timezone issue by introducing a `timezone` column on the `users` table and a selector in the user profile settings. `Habit` streak calculation and the `Dashboard`'s day grid now appropriately scope "today" and other dates via `now($user->timezone)`. Timezone feature has been thoroughly covered with newly written PHPUnit tests.
...
- Implemented User Timezone preferences so users can reliably toggle completions correctly bounded by their local time.
...
- Basic feature tests cover:
  ...
  - Asserting dashboard grid and streak boundaries behave according to user-selected timezone setting.
- Test coverage is robust enough for core operations.
