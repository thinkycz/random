...
**Summary:** Fixed the timezone issue where habits were being tracked based on the server's timezone. Added a `timezone` preference to user profiles, and updated dashboard dates, streak calculations, and completion toggles to respect the user's local timezone.
...
- Added user timezone handling so that habit streaks and daily dashboard views accurately reflect the user's local day boundaries instead of UTC.
...
- Basic feature tests cover:
  ...
  - Timezone boundaries for habit streaks and tracking completions.
