...
**Summary:** Addressed the timezone bug by adding a timezone field to user profiles. Habit completions, streak calculations, and daily boundaries now respect the individual user's timezone.
...
- Added timezone handling to the application, allowing users to configure their timezone in settings.
- Updated streak calculations and daily completion boundaries to respect the user's timezone rather than server UTC.
...
- Added `TimezoneTest` to explicitly verify completion logic and streak calculation using a mocked date/time in different timezones.
