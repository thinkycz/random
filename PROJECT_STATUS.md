...
**Summary:** Implemented user Timezone support. Users can now select their timezone in their profile settings. The system now records habit completions and calculates streaks based on the user's local timezone, fixing the issue where a server timezone discrepancy could cause habits to be recorded on the wrong day.
...
- Added `timezone` column to `users` table and incorporated timezone selection into the profile settings.
- Refactored Habit Completion logic to respect user timezones when evaluating day boundaries and streaks.
...
- Basic feature tests cover:
  ...
  - Timezone boundary testing: mocking local time across day boundaries to ensure streak logic remains consistent.
