...
**Summary:** Added timezone support for users. Users can now select their timezone in their profile, and habit completion tracking and streak calculations are now correctly based on the user's local time instead of the server time.
...
- Added user timezone support to fix issues where server time discrepancies caused incorrect tracking days.
...
- Basic feature tests cover:
  ...
  - Timezone handling for habit completion and streak calculation dates.
