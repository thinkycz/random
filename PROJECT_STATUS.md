...
**Summary:** Added user timezone support. Users can now select their timezone in their profile. "Today" calculations on the dashboard and streak calculations now respect the user's local timezone instead of the server's UTC time.
...
- Added client-side timezone handling to ensure date boundaries align with the user's local time.
