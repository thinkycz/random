...
**Summary:** Fixed the timezone bug. Added a `timezone` field to the User profile allowing users to define their local timezone. The application now uses this timezone to correctly determine "today" for habit completions and streak calculations, preventing users from unintentionally recording completions on the wrong day due to server offset.
...
- Implemented Client-Side Timezone Handling by adding `timezone` settings to user profiles and updating dashboard, toggling and streak queries.
...
- **UX:** Empty States need illustrations instead of raw text.
...
- Basic feature tests cover:
  ...
  - Timezone calculations when rendering and completing today's habits.
