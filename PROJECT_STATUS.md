...
**Summary:** Fixed the timezone bug where habit completion dates were based on the server's timezone. Added a `timezone` preference to the user profile, allowing users to accurately track their daily habit completions and streaks based on their local time.
...
- Added client-side timezone handling, fixing issues with incorrect local days for habits and streaks.
...
- **UI Improvements:** Improve empty states across the application using simple illustrations or SVGs.
...
- Basic feature tests cover:
  ...
  - Timezone-specific logic for habit toggling and streaks.
- Test coverage is strong for MVP, now including edge case handling for timezones.
