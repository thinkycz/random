...
**Summary:** Fixed the timezone bug. Added a `timezone` field to the user profile, defaulting to UTC. Dashboard, toggling completions, and streak calculations now properly respect the user's local timezone, ensuring accurate daily tracking across different geographical locations.
...
- Added user timezone support to correctly determine "today" boundaries.
...
- Basic feature tests cover:
  ...
  - Timezone day boundary calculations.
- Test coverage is good. Edge cases like timezone boundaries are now tested.
