...
**Summary:** Implemented client-side timezone handling. Users can now set their timezone in their profile settings. The dashboard, streak calculations, and completion toggles now accurately reflect the user's local time instead of the server's UTC time.
...
- Added timezone handling: users can set their timezone which updates day boundaries for tracking.
...
- **UI Updates:** Replace basic text empty states with simple illustrations or icons.

## Technical Debt Notes
- The controller logic for toggling and streaks has grown slightly with the timezone addition. Consider extracting to a service class.
...
- Basic feature tests cover:
  ...
  - Timezone handling during streak calculations and dashboard loading.
