...
**Summary:** Implemented client-side timezone handling. Users can now set their timezone in their profile. Habits and streaks now dynamically calculate their states based on the user's local timezone, rather than strictly on UTC server time.
...
- Added a `timezone` attribute to Users and updated all time/date logic (streaks, habit completion display) to consider the user's selected timezone.
...
- Basic feature tests cover:
  ...
  - Timezone testing logic for habit completion streaks.
- Test coverage is robust for the MVP features.
