...
**Summary:** Added timezone support for users. Users can now select their timezone in their profile settings, and habit completion tracking and streak calculations will respect their local time, fixing the issue of habits completing on the wrong day.
...
- Added timezone handling to the `users` table and `Habit` date calculations.
...
- Basic feature tests cover:
  ...
  - Timezone logic when toggling habit completions.
- Test coverage is moderate and includes edge cases like timezone handling.
