...
**Summary:** Addressed the timezone bug by allowing users to select and save their local timezone via their Profile Settings. Date calculations throughout the application now properly reflect the user's specific local timezone rather than defaulting to the server's UTC time.
...
- Added user timezone settings and fully supported local time zones for calculating completions, days, and streaks.
