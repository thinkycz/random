...
**Summary:** Fixed Timezone handling. Added a `timezone` preference to User profiles (defaults to UTC). The application's core logic for determining "today", the 7-day dashboard view, and streak calculations now properly respect the authenticated user's timezone.
...
- Added user timezone preference and integrated it across dashboard rendering, habit toggling, and streak calculations to resolve date-boundary issues.
...
- Basic feature tests cover:
  ...
  - Cross-timezone completion tracking boundaries.
- Test coverage is good. Timezone edge cases have been covered.
