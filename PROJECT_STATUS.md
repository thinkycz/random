...
**Summary:** Fixed the timezone issue where server-time date bounds were applied universally. Added a `timezone` preference to the User model, added a timezone selector to the Profile settings UI, and updated `HabitController` and `Habit` model to use the user's configured timezone for date calculations.
...
- Implemented Timezone setting in user profile and updated calculation logic to reflect user timezone.
